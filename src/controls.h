...
namespace Controls
{
	...
	{
		return WrapWidget(
			static_cast<bool (*)(const char *, float *, float, float, float, const char *, ImGuiSliderFlags)>(&ImGui::DragFloat),
			label, v, v_speed, v_min, v_max, format, flags);
	}

	// from https://github.com/ocornut/imgui/issues/1901 by @zfedoran.
	// Licensed under MIT: https://github.com/ocornut/imgui/issues/1901#issuecomment-755798471.
	...
	{
		return WrapWidget([](const char* label, float radius, int thickness, const ImU32& color) {
			using namespace ImGui;
			ImGuiWindow* window = GetCurrentWindow();
			if (window->SkipItems)
				return false;
		...
		WrapWidget(
			[](const char *str_id, bool *v)
			{
				...
				if (ImGui::IsItemHovered())
					...
					col_bg = ImGui::GetColorU32(ImLerp(ImGui::GetStyleColorVec4(ImGuiCol_FrameBg), ImGui::GetStyleColorVec4(ImGuiCol_CheckMark), t));
	...
	static bool KnobFloat(const char *label, float *p_value, float v_min, float v_max)
	{
		return WrapWidget(
			[](const char *label, float *p_value, float v_min, float v_max)
			{
				...
				draw_list->AddCircleFilled(center, radius_inner, ImGui::GetColorU32(is_active ? ImGuiCol_ButtonActive : is_hovered ? ImGuiCol_ButtonHovered
																																   : ImGuiCol_Button),
										   16);
